...
/* ──────────────────────────────────────────────────────────────────────────────
   Tri-State Konfig-Felder (inline, kein extra File nötig)
   Werte landen in steps[i].config und werden vom Runner/Backend ausgelesen.
   min_weight_*: Filtert schwache Einzelstimmen (0..1).
   label_threshold_*: leitet Label aus Score (−1..+1) ab.
──────────────────────────────────────────────────────────────────────────────── */
      ...
      <Box>
        <Box sx={{ mb:2, display:'flex', gap:1, alignItems:'center' }}>
          ...
          <Button startIcon={<AddIcon/>}
                  onClick={() => { setInsertPos(steps.length); setDraft({ id: uuid(), type:'ExtractionPrompt', promptId:0, active:true } as any); }}>
            Step
          </Button>
        </Box>
                  ...
                  <>
                    <TableRow key={r.step.id}>
                      ...
                      <TableCell>
                        ...
                        <IconButton size="small" onClick={() => { setInsertPos(idx + 1); setDraft({ ...(r.step as any), id: uuid() }); }}><AddIcon fontSize="small" /></IconButton>
                      </TableCell>
        ...
        <Drawer open={!!draft} onClose={() => setDraft(null)} anchor="right">
          {draft && (
              <Box sx={{ p:2, width:360, display:'flex', flexDirection:'column', gap:2 }}>
                <Select fullWidth value={draft.type} onChange={e=>{const val=e.target.value as string; setDraft({ ...(draft as any), type:val }); fetchPrompts(val);}}>
                  <MenuItem value="ExtractionPrompt">ExtractionPrompt</MenuItem>
                  <MenuItem value="ScoringPrompt">ScoringPrompt</MenuItem>
                  <MenuItem value="DecisionPrompt">DecisionPrompt</MenuItem>
                </Select>
                <Select fullWidth value={draft.promptId} onChange={e=>setDraft({ ...(draft as any), promptId:Number(e.target.value) })}>
