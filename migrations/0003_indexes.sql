-- Performance & Eindeutigkeit
CREATE INDEX idx_pdf_texts_pdf_page     ON pdf_texts(pdf_id, page_no);
...
-- Genau 1 finales Ergebnis pro Step-Instanz erzwingen:
CREATE UNIQUE INDEX uq_psa_final_per_runstep
    ON pipeline_step_attempts(run_step_id)
    WHERE is_final;

-- Schnelle JSONB-Suchen in Run-Summaries:
...
-- Ergänzende Indexe für neue Tabellen:
