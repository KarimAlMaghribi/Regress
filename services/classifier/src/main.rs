            ...
            match call_openai_chat(&awc_client, "gpt-4-turbo", vec![message]).await {
                Ok(answer) => {
                    ...
                    let _ = db_clone
                        .execute(
                            &stmt,
                            &[&id.to_string(), &prompt, &is_regress, &metrics, &responses],
                        )
                        .await;
    ...
    async fn classify_accepted() {
        ...
        if let Ok((client, connection)) = tokio_postgres::connect(&db_url, NoTls).await {
            ...
            client
                ...
                .await
                .unwrap();
            client.execute("CREATE TABLE IF NOT EXISTS classifications (id SERIAL PRIMARY KEY, run_time TIMESTAMPTZ DEFAULT now(), file_name TEXT, prompts TEXT, regress BOOLEAN NOT NULL, metrics JSONB NOT NULL, responses JSONB NOT NULL)", &[]).await.unwrap();
