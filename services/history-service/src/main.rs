    ...
    let _ = db
        .execute(
            "INSERT INTO classification_history (id, prompt, result, pdf_url, timestamp, status) \
             ...
             ON CONFLICT (id) DO UPDATE SET \
               prompt=EXCLUDED.prompt, result=EXCLUDED.result, pdf_url=EXCLUDED.pdf_url, timestamp=EXCLUDED.timestamp, status='completed'",
            &[
                &entry.id,
                &entry.prompt,
                &entry.result,
...
async fn latest(db: &tokio_postgres::Client, limit: i64) -> Vec<HistoryEntry> {
    let stmt = db
        .prepare("SELECT id, prompt, result, pdf_url, timestamp, status FROM classification_history ORDER BY timestamp DESC LIMIT $1")
        .await
        .unwrap();
...
async fn list_by_status(db: &tokio_postgres::Client, status: Option<String>) -> Vec<HistoryEntry> {
    let (sql, params): (&str, Vec<&(dyn tokio_postgres::types::ToSql + Sync)>) = if let Some(s) = &status {
        ("SELECT id, prompt, result, pdf_url, timestamp, status FROM classification_history WHERE status = $1 ORDER BY timestamp DESC", vec![s])
    } else {
        ("SELECT id, prompt, result, pdf_url, timestamp, status FROM classification_history ORDER BY timestamp DESC", vec![])
    };
...
async fn classifications(state: web::Data<AppState>, query: web::Query<HashMap<String, String>>) -> impl Responder {
    let limit = query
        .get("limit")
...
async fn analyses(state: web::Data<AppState>, query: web::Query<HashMap<String, String>>) -> impl Responder {
...
async fn ws_index(req: HttpRequest, stream: Payload, state: web::Data<AppState>) -> Result<HttpResponse, Error> {
    ...
    let ws = WsConn { db: state.db.clone(), rx };
    ws::start(ws, &req, stream)
}

async fn start_kafka(db: Arc<tokio_postgres::Client>, tx: broadcast::Sender<HistoryEntry>, settings: Settings, pdf_base: String) {
    let consumer: StreamConsumer = ClientConfig::new()
        .set("group.id", "history-service")
        .set("bootstrap.servers", &settings.message_broker_url)
        .create()
        .unwrap();
    consumer.subscribe(&["pdf-uploaded", "classification-result"]).unwrap();
        ...
        match consumer.recv().await {
            ...
            Ok(m) => {
                if let Some(Ok(payload)) = m.payload_view::<str>() {
                    match m.topic() {
                        "pdf-uploaded" => {
                            if let Ok(data) = serde_json::from_str::<shared::dto::PdfUploaded>(payload) {
                        ...
                        "classification-result" => {
                            if let Ok(data) = serde_json::from_str::<shared::dto::ClassificationResult>(payload) {
                                let entry = HistoryEntry {
                                    id: data.id.to_string(),
                                    prompt: Some(data.prompt.clone()),
                                    result: Some(serde_json::json!({"regress": data.regress, "answer": data.answer })),
                                    pdf_url: format!("{}/pdf/{}", pdf_base, data.id),
                                    timestamp: Utc::now(),
