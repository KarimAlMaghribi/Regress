...
mod consolidation; // <- NUR für Pipeline-Config; nicht mehr für Finals
use consolidation::{/* Konsolidierer werden NICHT mehr hier genutzt */};
...
fn slugify(s: &str) -> String {
    ...
    for ch in s.chars() {
        ...
        if c.is_ascii_alphanumeric() { out.push(c); last_us = false; }
...
fn key_for_prompt(items: &[shared::dto::PromptResult], prompt_id: i32) -> String {
    if let Some(k) = items.iter().find(|r| r.prompt_id == prompt_id).and_then(|r| r.json_key.clone()) {
        return slugify(&k);
    }
...
async fn init_db(pool: &PgPool) {
    // nur Pipeline-Tabelle absichern
...
async fn fetch_config(pool: &PgPool, id: Uuid) -> Result<PipelineConfig, HttpResponse> {
    ...
    let value: serde_json::Value = row.try_get("config_json")
...
async fn store_config(pool: &PgPool, id: Uuid, cfg: &PipelineConfig) -> Result<(), HttpResponse> {
    ...
    let res = sqlx::query("UPDATE pipelines SET name=$2, config_json=$3, updated_at=now() WHERE id=$1")
        .bind(id).bind(&cfg.name).bind(json)
        .execute(pool).await
        .map_err(|_| HttpResponse::InternalServerError().finish())?;
...
async fn list_pipelines(data: web::Data<AppState>) -> impl Responder {
    match sqlx::query("SELECT id, name, config_json FROM pipelines").fetch_all(&data.pool).await {
        Ok(rows) => {
            let res: Vec<PipelineInfo> = rows.into_iter().filter_map(|r| {
        ...
        Err(e) => { error!("db error: {}", e); HttpResponse::InternalServerError().finish() }
    }
}
...
async fn get_run(data: web::Data<AppState>, path: web::Path<uuid::Uuid>) -> impl Responder {
    ...
    let final_rows = match sqlx::query(
        ...
        "#
    ).bind(run_id).fetch_all(&data.pool).await {
        Ok(rows) => rows,
        Err(e) => {
    ...
    for r in final_rows {
        ...
        let key:   String = r.try_get::<Option<String>, _>("final_key").unwrap_or(None).unwrap_or_default();
        let val:   Value  = r.try_get("result").unwrap_or(json!({}));
        if key.is_empty() { continue; }
        match ptype.as_str() {
            "ExtractionPrompt" => { extracted.insert(key, val); }
            "ScoringPrompt"    => { scores.insert(key, val); }
            "DecisionPrompt"   => { decisions.insert(key, val); }
            _ => {}
        }
    }

    // kompaktes Log (alle Steps)
    ...
    let res_json = json!({
        ...
        "scores": scores,
        "decisions": decisions,
        "extraction": [],  // Arrays können (falls gewünscht) aus steps rekonstruiert werden
        "scoring":   [],
        "decision":  [],
        "log": steps
    });
...
async fn create_pipeline(
    ...
    match sqlx::query("INSERT INTO pipelines (id, name, config_json) VALUES ($1,$2,$3)")
        .bind(id).bind(&name).bind(jsonv).execute(&data.pool).await
    {
        Ok(_) => HttpResponse::Created().json(PipelineInfo { id, name, steps }),
        Err(e) => { error!("insert error: {}", e); HttpResponse::InternalServerError().finish() }
    }
}
...
async fn update_pipeline(
    ...
    match sqlx::query("UPDATE pipelines SET name=$2, config_json=$3, updated_at=now() WHERE id=$1")
        .bind(*path).bind(&cfg.name).bind(jsonv)
...
async fn delete_pipeline(data: web::Data<AppState>, path: web::Path<Uuid>) -> impl Responder {
    match sqlx::query("DELETE FROM pipelines WHERE id=$1")
        .bind(*path).execute(&data.pool).await
    {
        Ok(r) if r.rows_affected() == 1 => HttpResponse::NoContent().finish(),
        _ => HttpResponse::NotFound().finish(),
    }
...
async fn add_step(
    ...
    match fetch_config(&data.pool, *path).await {
        Ok(mut cfg) => {
            if input.index > cfg.steps.len() { return HttpResponse::BadRequest().finish(); }
...
async fn delete_step(data: web::Data<AppState>, path: web::Path<(Uuid, String)>) -> impl Responder {
    let (id, step_id) = path.into_inner();
    let mut cfg = match fetch_config(&data.pool, id).await { Ok(c) => c, Err(e) => return e };
    let orig_len = cfg.steps.len();
    cfg.steps.retain(|s| s.id != step_id);
    if cfg.steps.len() == orig_len { return HttpResponse::NotFound().finish(); }
    match store_config(&data.pool, id, &cfg).await {
        Ok(()) => HttpResponse::NoContent().finish(),
        Err(e) => e,
    }
...
async fn reorder_steps(
    ...
    let mut cfg = match fetch_config(&data.pool, *path).await { Ok(c) => c, Err(e) => return e };
    if input.order.len() != cfg.steps.len() { return HttpResponse::BadRequest().finish(); }
...
async fn run_pipeline(
    ...
    let row = match sqlx::query("SELECT pdf_id FROM uploads WHERE id=$1")
        .bind(input.file_id).fetch_one(&data.pool).await
    {
        Ok(r) => r, Err(_) => return HttpResponse::NotFound().finish(),
    };
    let pdf_id: i32 = match row.try_get("pdf_id") {
        Ok(v) => v, Err(_) => return HttpResponse::InternalServerError().finish(),
    };

    let _ = sqlx::query("UPDATE uploads SET pipeline_id=$1 WHERE id=$2")
        .bind(*path).bind(input.file_id).execute(&data.pool).await;
    ...
    // Nur publishen; keine Best-Effort-Konsolidierung mehr hier
    let _ = data.producer
        .send(FutureRecord::to("pipeline-run").payload(&payload).key(&()),
              Duration::from_secs(0))
        .await;

    HttpResponse::Accepted().json(json!({ "status": "queued", "pdf_id": pdf_id, "pipeline_id": *path }))
}
