...
async fn list_texts(db: web::Data<tokio_postgres::Client>) -> actix_web::Result<HttpResponse> {
    let stmt = db.prepare("SELECT pdf_id FROM pdf_texts ORDER BY pdf_id DESC")
...
async fn start_analysis(
    ...
    for id in req.ids {
        let stmt = db.prepare("SELECT text FROM pdf_texts WHERE pdf_id = $1")
            .await.map_err(actix_web::error::ErrorInternalServerError)?;
...
async fn main() -> std::io::Result<()> {
    ...
    let (db_client, connection) = tokio_postgres::connect(&db_url, NoTls).await
        .map_err(|e| { error!(%e, "db connect failed"); std::io::Error::new(std::io::ErrorKind::Other, "db") })?;
    ...
    db_client.execute("ALTER TABLE uploads ADD COLUMN IF NOT EXISTS pipeline_id UUID", &[])
        .await.ok();
    ...
    let consumer: StreamConsumer = ClientConfig::new()
        ...
        .create()
        .map_err(|e| { error!(%e, "consumer"); std::io::Error::new(std::io::ErrorKind::Other, "kafka") })?;
    consumer.subscribe(&["pdf-merged"]).map_err(|e| { error!(%e, "subscribe"); std::io::Error::new(std::io::ErrorKind::Other, "kafka") })?;
    info!("kafka consumer subscribed to pdf-merged");

    let producer: FutureProducer = ClientConfig::new()
        .set("bootstrap.servers", &settings.message_broker_url)
        .create()
        .map_err(|e| { error!(%e, "producer"); std::io::Error::new(std::io::ErrorKind::Other, "kafka") })?;
                        ...
                        match serde_json::from_str::<PdfUploaded>(payload) {
                            Ok(event) => {
                                ...
                                let stmt = match db.prepare("SELECT data FROM pdfs WHERE id = $1").await {
                                    Ok(s) => s,
                                    Err(e) => { error!(%e, "prepare failed"); continue; }
                                };
                                match db.query_one(&stmt, &[&event.pdf_id]).await {
                                    Err(e) => error!(%e, id = event.pdf_id, "no pdf data"),
                                    Ok(row) => {
                                        ...
                                        match text_result {
                                            ...
                                            Ok(text_raw) => {
                                                ...
                                                // upsert
                                                let stmt_up = match db.prepare(
                                                    "INSERT INTO pdf_texts (pdf_id, text) VALUES ($1, $2)
                                                     ON CONFLICT (pdf_id) DO UPDATE SET text = EXCLUDED.text"
                                                ).await {
                                                    Ok(s) => s, Err(e) => { error!(%e, "prepare upsert"); continue; }
                                                };
                                                if let Err(e) = db.execute(&stmt_up, &[&event.pdf_id, &text]).await {
